        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center h-16 px-6 border-b border-gray-200">
            <h1 className="text-xl font-bold text-[#3B82F6] font-poppins">
              {role === "student" ? "Student Portal" : "Tutor Portal"}
            </h1>
          </div>
  ...
  <div className="flex items-center">
    <div className="w-10 h-10 rounded-full bg-[#6366F1] flex items-center justify-center text-white">
      {user?.name ? (
        ...
        <span className="font-medium">{role === "student" ? "S" : "T"}</span>
      )}
    </div>
    <div className="ml-3">
      <p className="text-sm font-medium text-gray-800">
        {user?.name || (role === "student" ? "Student" : "Tutor")}
      </p>
      <p className="text-xs text-gray-500">
        {role === "student" ? "Student" : (user?.expertise?.length > 0 ? `${user.expertise[0]} Tutor` : "Tutor")}
      </p>
    </div>
  </div>
